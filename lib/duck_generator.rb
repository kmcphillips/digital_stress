...
module DuckGenerator
  ...
  def generate(server:, channel:, message:)
    ...
    OpenaiClient.chat(prompt(training_messages, message), model: "gpt-5-mini")&.first
  end
  ...
  def prompt(training_messages, message)
    fragments = []
    fragments << "Do not include '<@#{Global.bot.bot_user.id}>' in the answer." if Global.bot&.bot_user&.id

    "You are named 'Duck'. You are an outgoing quick witted character that gives brief responses. Your style is to respond with non sequeturs. Give me only a single line response, not many lines. You are participating in a casual chat with good friends on a Discord server. #{fragments.join(" ")} Reply to the following message:\n\n#{message}\n\nBut very important! Reply in the style of the following messages which are an example of the voice and tone and language you should use (each message is a single line):\n\n#{training_messages.join("\n")}."
  end
end
