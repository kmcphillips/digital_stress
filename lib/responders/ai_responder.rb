...
class AiResponder < BaseResponder
  ...
  def respond
    ...
    #     name, completion = MandateModels.question(text, name: from_name) if completion.blank?
    #
    #     if completion.blank?
    #       ":bangbang: Quack! After two attempts at n=2 all responses came back blank for question completion of #{ from_name }: #{ text }"
    #     else
